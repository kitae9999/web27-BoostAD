...
describe('CampaignServingSnapshotService', () => {
  ...
  const buildCampaign = (
    id: string,
    embeddingDocument: number[] | null = embedding
  ): CachedCampaign => ({
    id,
    userId: 1,
    servingVersion: 1,
    ...
    deletedAt: null,
    tags: ['tag'],
    embeddingModelVersion: 'Xenova/all-MiniLM-L6-v2@request-v1-mean-normalized',
    embeddingDocument: embeddingDocument ?? undefined,
  });
  ...
  it('builds the initial snapshot only once for concurrent reads', async () => {
    ...
    expect(first[0].embeddingDocument).toBeInstanceOf(Float32Array);
  ...
  it('repairs a campaign whose embeddings were generated by another process', async () => {
    const incomplete = buildCampaign('c1', null);
  ...
  it('bootstraps from a head, then replays events that arrived during full load', async () => {
    ...
    const getAllSearchCampaigns = jest.fn().mockResolvedValue([initial]);
    const findCampaignsByIds = jest.fn().mockResolvedValue([updated]);
    const searchRepository = {
      getAllSearchCampaigns,
      findCampaignsByIds,
    } as unknown as CampaignSearchRepository;
    const xrevrange = jest.fn().mockResolvedValue([['10-0', []]]);
    const searchRedis = {
      xrevrange,
      xrange: jest
        .fn()
    ...
    expect(getAllSearchCampaigns).toHaveBeenCalledTimes(1);
    expect(findCampaignsByIds).toHaveBeenCalledWith(['c1']);
  });
  ...
  it('lets each API snapshot apply the same independent Stream event', async () => {
    ...
    for (const instance of services) {
      ...
      expect(instance.findCampaignsByIds).toHaveBeenCalledTimes(1);
    }
  });
